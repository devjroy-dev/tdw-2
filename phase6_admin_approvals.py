"""
PHASE 6 — Admin: Discovery Approval Queue
Repo: tdw-2

Changes:
  1. Create web/app/admin/approvals/page.tsx
     — Pending submissions queue (vendors who hit Submit for Discovery)
     — Each row: tap → drawer slides in with full profile + all photos
     — Per-photo: Approve ✓ / Reject ✗ (with rejection note)
     — "Approve for Discovery" button — sets all 3 flags atomically, vendor goes live
     — "Deny Profile" button — sets rejection reason, vendor sees State 5 on Discover Dash
     — "Remove from Feed" for already-live vendors — clears all 3 flags
  2. Add "Approvals" to admin layout NAV under PEOPLE group

Run from: /workspaces/tdw-2
Command:  python3 phase6_admin_approvals.py
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# CHANGE 1 — Create admin/approvals/page.tsx
#
# Design decisions:
#   - Single page, two tabs: "Pending" and "Live Vendors"
#   - Pending tab = vendors who submitted but aren't live yet
#     (discover_submitted_at set, is_approved = false)
#   - Live tab = vendors who are currently live (all 3 flags true)
#     with a "Remove from Feed" button for each
#   - Drawer: slides in from right, shows hero + bio + all photos + key fields
#   - Per-photo approve/reject uses PATCH /api/v3/admin/images/:id
#   - Approve for Discovery uses PATCH /api/v3/admin/makers/:id with all 3 flags
#   - Atomic: is_approved + discover_listed + vendor_discover_enabled set together
# ─────────────────────────────────────────────────────────────────────────────
...
approvals_dir = 'web/app/admin/approvals'
os.makedirs(approvals_dir, exist_ok=True)
with open(f'{approvals_dir}/page.tsx', 'w') as f:
    f.write(APPROVALS_PAGE)
changes.append('✓ Change 1: Created web/app/admin/approvals/page.tsx — full discovery approval queue')


# ─────────────────────────────────────────────────────────────────────────────
# CHANGE 2 — Add "Discovery Approvals" to admin layout NAV under PEOPLE group
# ─────────────────────────────────────────────────────────────────────────────
LAYOUT_PATH = 'web/app/admin/layout.tsx'
with open(LAYOUT_PATH, 'r') as f:
    layout = f.read()

OLD_PEOPLE = """  { group: 'PEOPLE', items: [
    { label: 'Dreamers', path: '/admin/dreamers', icon: '♡' },
    { label: 'Makers', path: '/admin/makers', icon: '✦' },
    { label: 'Invite Codes', path: '/admin/invites', icon: '⌘' },
  ]},"""

NEW_PEOPLE = """  { group: 'PEOPLE', items: [
    { label: 'Dreamers',             path: '/admin/dreamers',  icon: '♡' },
    { label: 'Makers',               path: '/admin/makers',    icon: '✦' },
...
if OLD_PEOPLE in layout:
    layout = layout.replace(OLD_PEOPLE, NEW_PEOPLE)
    with open(LAYOUT_PATH, 'w') as f:
        f.write(layout)
    changes.append('✓ Change 2: "Discovery Approvals" added to admin nav under PEOPLE')
else:
    changes.append('✗ Change 2 FAILED — admin nav PEOPLE pattern not found')


# ─────────────────────────────────────────────────────────────────────────────
# Report
# ─────────────────────────────────────────────────────────────────────────────
print('\nPhase 6 — Admin Discovery Approval Queue — complete\n')
for c in changes:
    print(c)
print('\nNext: git add -A && git commit -m "Phase 6: admin discovery approval queue — review drawer, per-photo approve/reject, atomic approve" && git push')
