...
import { getVendorSession, setVendorSession, clearVendorSession } from '../../utils/session';

const GOLD = '#C9A84C';
const API = RAILWAY_URL;

export default function VendorPinLoginScreen() {
  ...
  useEffect(() => {
    getVendorSession().then(s => {
      if (!s?.id && !s?.vendorId && !s?.phone) { router.replace('/'); return; }
      if (!s?.pin_set) { router.replace('/(vendor)/pin'); return; }
    ...
    try {
      ...
      if (d.success) {
        await setVendorSession({
          ...session,
          vendorId: d.userId || session?.vendorId || session?.id,
          id: d.userId || session?.id,
          vendorName: d.name || session?.vendorName || session?.name || '',
          name: d.name || session?.name || session?.vendorName || '',
