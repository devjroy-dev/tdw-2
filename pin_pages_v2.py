"""
VENDOR PIN PAGES — v2 dark aesthetic rewrite
Repo: tdw-2

Rewrites:
  web/app/vendor/pin/page.tsx       — Set PIN (first time)
  web/app/vendor/pin-login/page.tsx — Enter PIN (returning)

Design: full-screen dark #0C0A09, carousel wedding images,
glass frosted bottom strip, same language as landing page.
After pin-login success → /vendor/today (not /vendor/dashboard).

Run from: /workspaces/tdw-2
Command:  python3 pin_pages_v2.py
"""

import os

# ── SLIDES (same as landing page) ─────────────────────────────────────────────
SLIDES = [
...
export default function VendorPinPage() {
  ...
  const handleInput = (idx: number, val: string, refs: React.MutableRefObject<(HTMLInputElement | null)[]>, setter: React.Dispatch<React.SetStateAction<string[]>>, isConfirm = false) => {
    if (!/^\\d*$/.test(val)) return;
    const v = val.slice(-1);
    setter(prev => { const n = [...prev]; n[idx] = v; return n; });
    if (v && idx < 3) refs.current[idx + 1]?.focus();
    if (v && idx === 3 && !isConfirm) {
      // Move to confirm stage
      setTimeout(() => { setStage('confirm'); setTimeout(() => confirmRefs.current[0]?.focus(), 80); }, 80);
    }
  };
  ...
  return (
    <>
      ...
      <div style={{ position:'fixed',inset:0,background:'#0C0A09',overflow:'hidden' }}>
        ...
        {/* Carousel */}
        {SLIDES.map((src, i) => (
          <div key={i} style={{ position:'absolute',inset:0,backgroundImage:\`url(\${src})\`,backgroundSize:'cover',backgroundPosition:'center',opacity: i === slide ? 0.55 : 0,transition:'opacity 1200ms ease',animation: i === slide ? 'imgFade 1200ms ease' : 'none' }} />
        ))}
        ...
        <div style={{ position:'absolute',bottom:0,left:0,right:0,animation:'pinFadeIn 400ms cubic-bezier(0.22,1,0.36,1)' }}>
          <div style={{
            ...
            padding:\`28px 32px calc(env(safe-area-inset-bottom, 16px) + 32px)\`,
          }}>
            {/* Brand */}
            <p style={{ fontFamily:"'Cormorant Garamond',serif",fontStyle:'italic',fontWeight:300,fontSize:15,color:'rgba(248,247,245,0.5)',margin:'0 0 2px',letterSpacing:'0.02em' }}>The Dream Wedding</p>
            ...
            {/* PIN inputs */}
            <div className={shaking ? 'pin-shake' : ''} style={{ display:'flex',justifyContent:'center',gap:16,marginBottom:32 }}>
              {(stage === 'pin' ? pin : confirm).map((d, i) => (
                <input
                  key={i}
                  ref={el => { (stage === 'pin' ? pinRefs : confirmRefs).current[i] = el; }}
                  type="tel" maxLength={1} value={d}
                  onChange={e => handleInput(i, e.target.value, stage === 'pin' ? pinRefs : confirmRefs, stage === 'pin' ? setPin : setConfirm, stage === 'confirm')}
                  onKeyDown={e => { if (e.key === 'Backspace') handleBackspace(i, d, stage === 'pin' ? pinRefs : confirmRefs, stage === 'pin' ? setPin : setConfirm); }}
                  style={inputStyle}
                  disabled={loading}
                />
              ))}
...
# ── VENDOR PIN LOGIN — /vendor/pin-login ──────────────────────────────────────
PIN_LOGIN = """\
'use client';
import { useEffect, useRef, useState, useCallback } from 'react';
import { useRouter } from 'next/navigation';
      ...
      <div style={{ position:'fixed',inset:0,background:'#0C0A09',overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute',bottom:0,left:0,right:0,animation:'pinFadeIn 400ms cubic-bezier(0.22,1,0.36,1)' }}>
          <div style={{
            ...
            {/* Forgot PIN */}
            <p
              onClick={() => { localStorage.removeItem('vendor_web_session'); localStorage.removeItem('vendor_session'); router.replace('/'); }}
...
# Write both files
os.makedirs('web/app/vendor/pin', exist_ok=True)
os.makedirs('web/app/vendor/pin-login', exist_ok=True)

with open('web/app/vendor/pin/page.tsx', 'w') as f:
    f.write(PIN_SET)
print('✓ web/app/vendor/pin/page.tsx — rewritten (dark, carousel, gold PIN inputs)')

with open('web/app/vendor/pin-login/page.tsx', 'w') as f:
    f.write(PIN_LOGIN)
print('✓ web/app/vendor/pin-login/page.tsx — rewritten (dark, carousel, gold PIN inputs)')

print('\nBoth PIN pages now:')
print('  - Full screen dark #0C0A09 background')
print('  - Wedding image carousel at 55% opacity')
print('  - Glass frosted bottom strip (matches landing page)')
print('  - Gold underline PIN inputs on dark background')
print('  - Pin set → /vendor/today (not /vendor/dashboard)')
print('  - Pin login → /vendor/today')
print('\nNext: git add -A && git commit -m "Design: vendor PIN pages v2 — dark, carousel, glass strip" && git push')
