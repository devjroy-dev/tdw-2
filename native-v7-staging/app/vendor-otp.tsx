...
import { setVendorSession } from '../utils/session';
...
export default function VendorOtpScreen() {
  ...
  const submitOtp = useCallback(async (code: string) => {
    ...
    try {
      const res = await fetch(`${RAILWAY_URL}/api/v2/vendor/auth/verify-otp`, {
        method: 'POST',
      ...
      // Vendor not found — show holding screen, do not crash
      if (res.status === 404 || data.notFound || data.status === 'not_found') {
        setShowHolding(true);
        return;
      }
  ...
  async function handleResend() {
    if (resendSeconds > 0) return;
    try {
      await fetch(`${RAILWAY_URL}/api/v2/vendor/auth/send-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  // Holding screen — vendor number not in system
  if (showHolding) {
    return (
      <View style={[styles.container, styles.holdingContainer, { paddingTop: insets.top }]}>
        <Text style={styles.holdingSparkle}>✦</Text>
        <Text style={styles.holdingTitle}>Your account is being set up.</Text>
        <Text style={styles.holdingBody}>
          We'll be in touch shortly.
        </Text>
        <TouchableOpacity
          style={styles.holdingBackButton}
          onPress={() => router.replace('/')}
          activeOpacity={0.85}
        >
          <Text style={styles.holdingBackText}>BACK TO HOME</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  if (showBiometricOffer) {
    return (
      <View style={[styles.container, styles.biometricContainer]}>
        ...
        <TouchableOpacity onPress={() => router.replace('/(vendor)/today')}>
