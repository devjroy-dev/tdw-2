  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 9, color: '#555250', letterSpacing: '0.25em', textTransform: 'uppercase', marginBottom: 6 }}>Collab Hub</div>
      ...
      {loading ? (
        ...
        <div style={{ padding: '60px 0', textAlign: 'center', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 13, color: '#555250' }}>When Makers start collaborating, it will appear here.</div>
      ) : (
                  ...
                  <tr key={p.id} style={{ borderTop: '1px solid #F0EEE8', background: p.is_flagged ? '#FFF8F8' : 'transparent', borderLeft: p.is_flagged ? '2px solid #C0392B' : '2px solid transparent' }}>
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        <button onClick={() => toggleFlag(p.id)} style={{ background: 'none', border: 'none', fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 8, letterSpacing: '0.18em', textTransform: 'uppercase', color: p.is_flagged ? '#C0392B' : '#555250', cursor: 'pointer' }}>{p.is_flagged ? 'Unflag' : 'Flag'}</button>
                        {p.status !== 'closed' && (
                          <button onClick={() => close(p.id)} style={{ background: 'none', border: 'none', fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 8, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#555250', cursor: 'pointer' }}>Close</button>
                        )}
                      </div>
                    </td>
                  </tr>
