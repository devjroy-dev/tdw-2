    ...
    <>
      ...
      {rejectTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#FFFFFF', borderRadius: 16, padding: 28, maxWidth: 360, width: '100%' }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontWeight: 300, color: '#111111', margin: '0 0 16px' }}>Reject Image</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
              {REJECT_REASONS.map(r => (
                <button key={r} onClick={() => setRejectReason(r)} style={{ padding: '10px 14px', background: rejectReason === r ? '#111111' : '#F4F1EC', color: rejectReason === r ? '#FFFFFF' : '#111111', border: 'none', borderRadius: 8, fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, cursor: 'pointer', textAlign: 'left' }}>{r}</button>
              ))}
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { if (rejectReason) reject(rejectTarget, rejectReason); }} disabled={!rejectReason} style={{ flex: 1, height: 44, background: rejectReason ? '#9B4545' : '#E2DED8', color: '#111111', border: 'none', borderRadius: 100, fontFamily: "'Jost',sans-serif", fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: rejectReason ? 'pointer' : 'default' }}>Reject</button>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#111111', margin: 0 }}>Image Approvals <span style={{ fontSize: 18, color: images.length > 0 ? '#C9A84C' : '#888580' }}>({images.length} pending)</span></p>
      ...
      {loading ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
          {[1,2,3,4,5,6].map(i => <div key={i} style={{ height: 200, borderRadius: 12, background: 'linear-gradient(90deg,#F0EEE8 25%,#E8E5DF 50%,#F0EEE8 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />)}
        </div>
      ) : images.length === 0 ? (
        ...
        Object.entries(byVendor).map(([vendorId, imgs]) => (
          <div key={vendorId} style={{ marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div>
                <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 300, color: '#111111', margin: '0 0 2px' }}>{imgs[0].vendor_name}</p>
              ...
              <button onClick={() => approveAllForVendor(vendorId)} style={{ height: 32, padding: '0 14px', background: '#4A7C59', color: '#111111', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 8, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: 'pointer' }}>Approve All</button>
            </div>
                  ...
                  <div style={{ height: 160, backgroundImage: `url(${img.url})`, backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative' }}>
                    {img.tags && img.tags.length > 0 && (
                      <div style={{ position: 'absolute', top: 6, left: 6, background: 'rgba(0,0,0,0.6)', borderRadius: 4, padding: '2px 6px' }}>
                        <span style={{ fontFamily: "'Jost',sans-serif", fontSize: 7, color: '#111111', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{img.tags[0]}</span>
                      </div>
                    )}
                  </div>
                  <div style={{ padding: '8px 10px', display: 'flex', gap: 6 }}>
                    <button onClick={() => approve(img.id)} disabled={processing === img.id} style={{ flex: 1, height: 28, background: '#4A7C59', color: '#111111', border: 'none', borderRadius: 6, fontFamily: "'Jost',sans-serif", fontSize: 8, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', opacity: processing === img.id ? 0.6 : 1 }}>✓</button>
