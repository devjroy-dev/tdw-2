...
app.get('/api/v2/couple/money/:userId', async (req, res) => {
  ...
  try {
    ...
    const upcoming = expenses.filter(e => e.due_date && new Date(e.due_date) >= now && new Date(e.due_date) <= in30 && e.status !== 'paid');
    const thisWeek = upcoming.filter(e => new Date(e.due_date) <= endOfWeek);

    res.json({ totalBudget: profile?.total_budget || 0, committed, paid, events, thisWeek, next30: upcoming });
  } catch (err) {
    res.status(500).json({ error: 'Internal server error' });
  }
});
