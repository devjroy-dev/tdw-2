...
export default function Home() {
  ...
  useEffect(() => {
    fetch(`${BACKEND}/api/v2/cover-photos`)
      .then(r=>r.json()).then(d=>{ if(d.photos?.length) setSlides(d.photos.map((p:any)=>p.image_url)); })
      .catch(()=>{});
    startCarousel();
  ...
  const reverseThreshold = useCallback(()=>{
    ...
    setTimeout(()=>{ startCarousel(); setScreen(role==='Dreamer'?'dreamer':'maker'); }, 1800);
  },[role, startCarousel]);

  const swapTag = (to: Role) => {
    setTagVis(false);
    setTimeout(()=>{ setTagText(to==='Maker'?MAKER_TAG:DREAMER_TAG); setTagVis(true); }, 600);
  };
  ...
  const sendOtp = async()=>{
    try {
      await fetch(`${BACKEND}/api/v2/couple/auth/send-otp`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:phone.replace(/\D/g,'')})});
  ...
  const verifyOtp = async()=>{
    try {
      const res = await fetch(`${BACKEND}/api/v2/couple/auth/verify-otp`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:phone.replace(/\D/g,''),code:otp.join('')})});
  ...
  const submitRequest = async()=>{
    try {
      await fetch(`${BACKEND}/api/v2/waitlist`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,instagram,role})});
      setReqDone(true);
  ...
  return (
    <div style={{...S, overflow:'hidden', background:'#0C0A09'}}>
      ...
      {/* BOTTOM ANCHOR — tagline pill + panel, always at bottom */}
      {isDark && (
        <div style={{
          ...
          {/* TAGLINE PILL — tight wrap, only as big as the text */}
          {(screen==='landing') && (
            <div
              onClick={()=>{ setPanelUp(true); setScreen('panel'); }}
              style={{
            ...
            >
              {screen==='panel' && (
                <>
                  ...
                  <p style={{textAlign:'center', fontFamily:'"Jost",sans-serif', fontWeight:200, fontSize:9, color:'rgba(248,247,245,0.4)', letterSpacing:'0.22em', textTransform:'uppercase', marginBottom:14}}>Are you a</p>
                  <div style={{display:'flex', gap:10}}>
                    <button onClick={()=>selectRole('Dreamer')} style={{...btnBase, flex:1, padding:'14px 0', background:'#C9A84C', color:'#0C0A09'}}>Dreamer</button>
                    <button onClick={()=>selectRole('Maker')} style={{...btnBase, flex:1, padding:'14px 0', background:'transparent', border:'0.5px solid rgba(248,247,245,0.25)', color:'#F8F7F5'}}>Maker</button>
                  </div>
                  <button onClick={()=>{ setPanelUp(false); setScreen('landing'); }} style={{...btnBase, display:'block', margin:'14px auto 0', background:'none', color:'rgba(248,247,245,0.3)', fontSize:8}}>← dismiss</button>
                </>
              )}

              {(screen==='dreamer'||screen==='maker') && (
                <>
                  <button onClick={()=>setScreen('panel')} style={{background:'none', border:'none', color:'rgba(248,247,245,0.5)', fontFamily:'"Jost",sans-serif', fontWeight:200, fontSize:18, cursor:'pointer', marginBottom:12, touchAction:'manipulation'}}>←</button>
      ...
      {/* CREAM SCREENS */}
      {creamVis && (
        <div style={{...S, zIndex:30, background:'#F8F7F5',
          display:'flex', flexDirection:'column', alignItems:'center',
          justifyContent:'center', padding:'44px 26px 36px',
          opacity:creamOp,
