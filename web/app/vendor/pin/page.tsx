...
export default function VendorPinPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (s?.pin_set) { router.replace('/vendor/dashboard'); return; }
    } catch {}
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (d.success) {
        localStorage.setItem('vendor_web_session', JSON.stringify({ ...session, pin_set: true }));
  ...
  const skip = () => { router.replace('/vendor/dashboard'); };
            ...
            <div className={shakingConfirm ? 'pin-shake' : ''} style={{ display: 'flex', justifyContent: 'center', gap: 12 }}>
              {confirm.map((d, i) => (
                <input
                  ...
                  onKeyDown={e => { if (e.key === 'Backspace') handleBackspace(i, d, confirmRefs, setConfirm); if (e.key === 'Enter' && allFilled) submit(); }}
                  style={boxStyle()}
                />
              ))}
            </div>
