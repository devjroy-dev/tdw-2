  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#111', margin: 0 }}>Messages & Moderation</p>
      </div>
      ...
      {tab === 'flagged' && (
        loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[1,2,3].map(i => <div key={i} style={{ height: 80, background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 12, backgroundImage: 'linear-gradient(90deg,#F8F7F5 25%,#F0EEE8 50%,#F8F7F5 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />)}
          </div>
        ) : flagged.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {flagged.map(msg => (
              <div key={msg.id} style={{ background: '#FFFFFF', border: '1px solid #FFEBEE', borderLeft: '3px solid #9B4545', borderRadius: '0 12px 12px 0', padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 400, color: '#111', margin: 0 }}>
                    {msg.couple_name} → {msg.vendor_name}
                  </p>
                  <span style={{ fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, color: '#888580' }}>{timeAgo(msg.created_at)}</span>
                </div>
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, color: '#555250', margin: '0 0 10px', lineHeight: 1.5 }}>
                  {msg.content.replace(/\[ contact hidden \]/g, '<span style="color:#9B4545;font-weight:400">[ contact hidden ]</span>')}
                </p>
                <a href={`/admin/messages/thread/${msg.enquiry_id}`} style={{ fontFamily: "'Jost',sans-serif", fontSize: 8, fontWeight: 300, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#888580', textDecoration: 'underline' }}>View full thread →</a>
              </div>
            ))}
          </div>
        )
      ...
      {tab === 'threads' && (
        <>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search threads…" style={{ width: '100%', maxWidth: 340, border: 'none', borderBottom: '1px solid #E2DED8', background: 'transparent', fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, color: '#111', padding: '8px 0', outline: 'none', marginBottom: 16 }} />
              ...
              <thead>
                <tr style={{ background: '#F8F7F5' }}>
                  {['Preview', 'Status', 'Last Activity', 'Created'].map(col => (
                    <th key={col} style={{ padding: '10px 14px', textAlign: 'left', fontFamily: "'Jost',sans-serif", fontWeight: 200, fontSize: 8, color: '#888580', letterSpacing: '0.2em', textTransform: 'uppercase' }}>{col}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {loading && [1,2,3].map(i => <tr key={i}><td colSpan={4} style={{ padding: '12px 14px' }}><div style={{ height: 14, background: 'linear-gradient(90deg,#EEECE8 25%,#F4F2EE 50%,#EEECE8 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 4 }} /></td></tr>)}
                {!loading && filteredThreads.map(t => (
                  <tr key={t.id} style={{ borderTop: '0.5px solid #F0EEE8' }}>
                    <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, color: '#111', maxWidth: 300 }}>
                    ...
                    <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: '#888580', whiteSpace: 'nowrap' }}>{timeAgo(t.last_message_at)}</td>
                    <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: '#888580', whiteSpace: 'nowrap' }}>{fmtDate(t.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
