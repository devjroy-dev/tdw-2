  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#111111', margin: 0 }}>Messages & Moderation</p>
      </div>
      ...
      {tab === 'flagged' && (
        loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[1,2,3].map(i => <div key={i} style={{ height: 80, background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 12, backgroundImage: 'linear-gradient(90deg,#1E1C1A 25%,#2A2825 50%,#1E1C1A 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />)}
          </div>
        ) : flagged.length === 0 ? (
            ...
            {flagged.map(msg => (
              <div key={msg.id} style={{ background: '#FFFFFF', border: '1px solid #FFEBEE', borderLeft: '3px solid #9B4545', borderRadius: '0 12px 12px 0', padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 400, color: '#111111', margin: 0 }}>
      ...
      {tab === 'threads' && (
        <>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search threads…" style={{ width: '100%', maxWidth: 340, border: 'none', borderBottom: '1px solid rgba(248,247,245,0.08)', background: 'transparent', fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, color: '#111111', padding: '8px 0', outline: 'none', marginBottom: 16 }} />
          <div style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 12, overflow: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
              ...
              <tbody>
                {loading && [1,2,3].map(i => <tr key={i}><td colSpan={4} style={{ padding: '12px 14px' }}><div style={{ height: 14, background: 'linear-gradient(90deg,#F0EEE8 25%,#E8E5DF 50%,#F0EEE8 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 4 }} /></td></tr>)}
                {!loading && filteredThreads.map(t => (
                  <tr key={t.id} style={{ borderTop: '0.5px solid #E8E5DF' }}>
                    <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, color: '#111111', maxWidth: 300 }}>
