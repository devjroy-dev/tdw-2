...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 14, padding: '20px 20px 18px' }}>
      <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 36, fontWeight: 300, color: '#111', margin: '0 0 4px', lineHeight: 1 }}>{value}</p>
    ...
    <>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#111', margin: 0 }}>Money & Revenue</p>
      ...
      {loading ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 20 }}>
          {[1,2,3,4].map(i => <div key={i} style={{ height: 90, borderRadius: 14, background: 'linear-gradient(90deg,#EEECE8 25%,#F4F2EE 50%,#EEECE8 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />)}
        </div>
      ) : (
        <>
          {/* Stat cards */}
          ...
          <div style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 14, padding: 20, marginBottom: 20 }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 300, color: '#111', margin: '0 0 16px' }}>Subscriptions by Tier</p>
            <div style={{ display: 'flex', gap: 0 }}>
              {[
                ...
                { tier: 'Prestige', count: data?.subscriptions_by_tier?.prestige || 0, color: '#111', bg: '#111' },
              ].map((t, i) => (
          ...
          {data?.monthly_chart && data.monthly_chart.length > 0 && (
            <div style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 14, padding: 20 }}>
              <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 300, color: '#111', margin: '0 0 20px' }}>Monthly Revenue — Last 12 Months</p>
