...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div style={{ background: '#161412', border: '1px solid rgba(248,247,245,0.1)', borderRadius: 14, padding: '20px 20px 18px' }}>
      <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 36, fontWeight: 300, color: '#F8F7F5', margin: '0 0 4px', lineHeight: 1 }}>{value}</p>
      <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 8, fontWeight: 200, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(248,247,245,0.4)', margin: 0 }}>{label}</p>
    ...
    <>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#F8F7F5', margin: 0 }}>Money & Revenue</p>
          <button onClick={exportCSV} style={{ height: 36, padding: '0 16px', background: 'transparent', border: '1px solid rgba(248,247,245,0.1)', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(248,247,245,0.55)', cursor: 'pointer' }}>↓ Export CSV</button>
        </div>
      </div>

      {loading ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 20 }}>
          {[1,2,3,4].map(i => <div key={i} style={{ height: 90, borderRadius: 14, background: 'linear-gradient(90deg,#1E1C1A 25%,#2A2825 50%,#1E1C1A 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />)}
        </div>
      ) : (
        <>
          {/* Stat cards */}
          ...
          <div style={{ background: '#161412', border: '1px solid rgba(248,247,245,0.1)', borderRadius: 14, padding: 20, marginBottom: 20 }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 300, color: '#F8F7F5', margin: '0 0 16px' }}>Subscriptions by Tier</p>
            <div style={{ display: 'flex', gap: 0 }}>
              {[
                { tier: 'Essential', count: data?.subscriptions_by_tier?.essential || 0, color: 'rgba(248,247,245,0.4)', bg: '#F4F1EC' },
                { tier: 'Signature', count: data?.subscriptions_by_tier?.signature || 0, color: '#C9A84C', bg: 'rgba(201,168,76,0.1)' },
                { tier: 'Prestige', count: data?.subscriptions_by_tier?.prestige || 0, color: '#F8F7F5', bg: '#111' },
              ].map((t, i) => (
          ...
          {data?.monthly_chart && data.monthly_chart.length > 0 && (
            <div style={{ background: '#161412', border: '1px solid rgba(248,247,245,0.1)', borderRadius: 14, padding: 20 }}>
              <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 300, color: '#F8F7F5', margin: '0 0 20px' }}>Monthly Revenue — Last 12 Months</p>
                ...
                {data.monthly_chart.map((m, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flex: 1, minWidth: 36 }}>
                      <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, fontWeight: 300, color: 'rgba(248,247,245,0.4)', margin: 0, whiteSpace: 'nowrap' }}>{m.revenue > 0 ? fmtINR(m.revenue) : ''}</p>
                      ...
                      <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 7, fontWeight: 200, letterSpacing: '0.1em', color: 'rgba(248,247,245,0.4)', margin: 0, whiteSpace: 'nowrap' }}>{m.month}</p>
                    </div>
                  );
                })}
              </div>
