    ...
    <>
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#FFFFFF', borderRadius: 16, padding: 28, maxWidth: 380, width: '100%' }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontWeight: 300, color: '#111', margin: '0 0 8px' }}>Delete this Maker?</p>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 340, background: '#FFFFFF', borderLeft: '1px solid #E2DED8', zIndex: 301, overflowY: 'auto' }}>
            <div style={{ padding: '20px 20px 16px', borderBottom: '0.5px solid #E2DED8' }}>
              ...
              <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 24, fontWeight: 300, color: '#111', margin: '0 0 4px' }}>{selected.name}</p>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <button onClick={() => updateMaker(selected.id, { is_verified: !selected.is_verified })} style={{ height: 40, background: '#F4F1EC', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#111', cursor: 'pointer' }}>
                  {selected.is_verified ? 'Unverify' : 'Verify Maker'}
                </button>
                <button onClick={() => updateMaker(selected.id, { is_luxury: !selected.is_luxury, luxury_approved: !selected.is_luxury })} style={{ height: 40, background: '#F4F1EC', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#111', cursor: 'pointer' }}>
                  {selected.is_luxury ? 'Remove Luxury' : 'Approve as Luxury'}
                </button>
                <button onClick={() => updateMaker(selected.id, { discover_enabled: !selected.discover_enabled })} style={{ height: 40, background: '#F4F1EC', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#111', cursor: 'pointer' }}>
                  {selected.discover_enabled ? 'Disable Discovery' : 'Enable Discovery'}
                </button>
                <button onClick={() => approveAllImages(selected.id)} style={{ height: 40, background: '#F4F1EC', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#111', cursor: 'pointer' }}>Approve All Images</button>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#111', margin: 0 }}>Makers <span style={{ fontSize: 18, color: '#888580' }}>({makers.length})</span></p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, phone, or city…" style={{ flex: 1, minWidth: 200, border: 'none', borderBottom: '1px solid #E2DED8', background: 'transparent', fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, color: '#111', padding: '8px 0', outline: 'none' }} />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 750 }}>
          ...
          <tbody>
            {loading && [1,2,3,4,5].map(i => (
              <tr key={i}><td colSpan={7} style={{ padding: '12px 14px' }}><div style={{ height: 14, background: 'linear-gradient(90deg,#EEECE8 25%,#F4F2EE 50%,#EEECE8 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 4 }} /></td></tr>
            ))}
            ...
            {!loading && makers.map(m => (
              <tr key={m.id} onClick={() => setSelected(m)} style={{ borderTop: '0.5px solid #F0EEE8', cursor: 'pointer' }}
                onMouseEnter={e => (e.currentTarget as HTMLTableRowElement).style.background = '#FAFAF8'}
                onMouseLeave={e => (e.currentTarget as HTMLTableRowElement).style.background = 'transparent'}
              >
                <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontWeight: 400, fontSize: 13, color: '#111', whiteSpace: 'nowrap' }}>{m.name || '—'}</td>
