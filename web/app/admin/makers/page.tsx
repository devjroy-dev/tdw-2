    ...
    <>
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#161412', borderRadius: 16, padding: 28, maxWidth: 380, width: '100%' }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontWeight: 300, color: '#F8F7F5', margin: '0 0 8px' }}>Delete this Maker?</p>
            ...
            <input value={deleteInput} onChange={e => setDeleteInput(e.target.value)} style={{ width: '100%', border: '1px solid rgba(248,247,245,0.1)', borderRadius: 8, padding: '10px 12px', fontFamily: "'DM Sans',sans-serif", fontSize: 14, outline: 'none', marginBottom: 16 }} placeholder="Type DELETE to confirm" />
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { if (deleteInput === 'DELETE') deleteMaker(deleteConfirm); }} disabled={deleteInput !== 'DELETE'} style={{ flex: 1, height: 44, background: deleteInput === 'DELETE' ? '#9B4545' : '#E2DED8', color: '#FFFFFF', border: 'none', borderRadius: 100, fontFamily: "'Jost',sans-serif", fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: deleteInput === 'DELETE' ? 'pointer' : 'default' }}>Delete Permanently</button>
      ...
      {selected && (
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 340, background: '#161412', borderLeft: '1px solid #E2DED8', zIndex: 301, overflowY: 'auto' }}>
            <div style={{ padding: '20px 20px 16px', borderBottom: '0.5px solid rgba(248,247,245,0.08)' }}>
              ...
              <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 24, fontWeight: 300, color: '#F8F7F5', margin: '0 0 4px' }}>{selected.name}</p>
              <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, color: 'rgba(248,247,245,0.4)', margin: 0 }}>{selected.category}{selected.city ? ` · ${selected.city}` : ''}</p>
            </div>
            <div style={{ padding: 20 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                {[
                  ...
                  <div key={r.l} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '0.5px solid rgba(248,247,245,0.06)' }}>
                    <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 200, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(248,247,245,0.4)', margin: 0 }}>{r.l}</p>
                    <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 300, color: '#F8F7F5', margin: 0 }}>{r.v}</p>
                  </div>
                ))}
              </div>

              {/* Tier change */}
              <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 200, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(248,247,245,0.4)', margin: '0 0 8px' }}>Change Tier</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <button onClick={() => updateMaker(selected.id, { is_verified: !selected.is_verified })} style={{ height: 40, background: '#1E1C1A', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#F8F7F5', cursor: 'pointer' }}>
                  {selected.is_verified ? 'Unverify' : 'Verify Maker'}
                </button>
                <button onClick={() => updateMaker(selected.id, { is_luxury: !selected.is_luxury, luxury_approved: !selected.is_luxury })} style={{ height: 40, background: '#1E1C1A', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#F8F7F5', cursor: 'pointer' }}>
                  {selected.is_luxury ? 'Remove Luxury' : 'Approve as Luxury'}
                </button>
                <button onClick={() => updateMaker(selected.id, { discover_enabled: !selected.discover_enabled })} style={{ height: 40, background: '#1E1C1A', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#F8F7F5', cursor: 'pointer' }}>
                  {selected.discover_enabled ? 'Disable Discovery' : 'Enable Discovery'}
                </button>
                <button onClick={() => approveAllImages(selected.id)} style={{ height: 40, background: '#1E1C1A', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#F8F7F5', cursor: 'pointer' }}>Approve All Images</button>
                {selected.phone && <button onClick={() => sendWA(selected.phone)} style={{ height: 40, background: '#25D366', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#FFFFFF', cursor: 'pointer' }}>Send WhatsApp</button>}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#F8F7F5', margin: 0 }}>Makers <span style={{ fontSize: 18, color: 'rgba(248,247,245,0.4)' }}>({makers.length})</span></p>
        </div>
        <button onClick={() => router.push('/admin/images')} style={{ height: 36, padding: '0 16px', background: '#111', color: '#F8F7F5', border: 'none', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer' }}>⬡ Approve Images</button>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 750 }}>
          ...
          <tbody>
            ...
            {!loading && makers.length === 0 && (
              <tr><td colSpan={7} style={{ padding: '40px', textAlign: 'center', fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 300, fontStyle: 'italic', color: 'rgba(248,247,245,0.4)' }}>No Makers found.</td></tr>
            )}
            {!loading && makers.map(m => (
              <tr key={m.id} onClick={() => setSelected(m)} style={{ borderTop: '0.5px solid rgba(248,247,245,0.06)', cursor: 'pointer' }}
                onMouseEnter={e => (e.currentTarget as HTMLTableRowElement).style.background = '#1E1C1A'}
                onMouseLeave={e => (e.currentTarget as HTMLTableRowElement).style.background = 'transparent'}
              >
                <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontWeight: 400, fontSize: 13, color: '#F8F7F5', whiteSpace: 'nowrap' }}>{m.name || '—'}</td>
                <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontWeight: 300, fontSize: 12, color: 'rgba(248,247,245,0.55)' }}>{m.category || '—'}</td>
                <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontWeight: 300, fontSize: 12, color: 'rgba(248,247,245,0.55)' }}>{m.city || '—'}</td>
                ...
                <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontWeight: 300, fontSize: 11, color: 'rgba(248,247,245,0.4)', whiteSpace: 'nowrap' }}>{fmtDate(m.created_at)}</td>
              </tr>
            ))}
          </tbody>
        </table>
