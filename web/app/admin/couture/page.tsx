...
export default function AdminCouturePage() {
  ...
  const pillBtn = (on: boolean, onClick: () => void, onL: string, offL: string) => (
    <button onClick={onClick} style={{ border: `0.5px solid ${on ? '#C9A84C' : '#E2DED8'}`, background: on ? 'rgba(201,168,76,0.08)' : 'transparent', color: on ? '#C9A84C' : '#888580', fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 7, letterSpacing: '0.18em', textTransform: 'uppercase' as const, padding: '3px 8px', borderRadius: 20, cursor: 'pointer' }}>{on ? onL : offL}</button>
  );
  ...
  return (
    <>
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9998 }}>
          <div style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 6, padding: 28, maxWidth: 340, width: '90%' }}>
            ...
            <div style={{ fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 13, color: '#888580', marginBottom: 24 }}>This will hide the product from the catalog.</div>
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button onClick={() => setConfirmDelete(null)} style={{ background: 'none', border: 'none', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 13, color: '#888580', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 9997 }}>
          <div style={{ background: '#FFFFFF', borderRadius: '8px 8px 0 0', padding: 28, width: '100%', maxWidth: 480, maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button onClick={() => setShowAdd(false)} style={{ background: 'none', border: 'none', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 13, color: '#888580', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 28 }}>
        <div>
          <div style={{ fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 9, color: '#888580', letterSpacing: '0.25em', textTransform: 'uppercase', marginBottom: 6 }}>Couture</div>
      ...
      {loading ? (
        ...
        <div style={{ padding: '60px 0', textAlign: 'center', fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 13, color: '#888580' }}>The atelier awaits its first creation.</div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>
          {filtered.map(p => (
            <div key={p.id} style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 6, overflow: 'hidden', animation: 'fadeIn 0.3s ease' }}>
              {p.images?.[0] ? (
                ...
                <div style={{ height: 120, background: '#F0EEE8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 8, color: '#888580', letterSpacing: '0.2em', textTransform: 'uppercase' }}>No Image</span>
                </div>
              )}
              <div style={{ padding: '12px 14px' }}>
                <div style={{ fontFamily: '"DM Sans", sans-serif', fontWeight: 300, fontSize: 10, color: '#888580', marginBottom: 2 }}>{p.designer_name}</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <span style={{ fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 7, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#888580', background: '#F0EEE8', padding: '2px 7px', borderRadius: 10 }}>{p.category}</span>
                ...
                <button onClick={() => setConfirmDelete(p.id)} style={{ background: 'none', border: 'none', fontFamily: '"Jost", sans-serif', fontWeight: 200, fontSize: 7, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#888580', cursor: 'pointer', opacity: 0.6 }}>Remove</button>
              </div>
            </div>
          ))}
        </div>
