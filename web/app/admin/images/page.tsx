    ...
    <>
      ...
      {rejectTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#161412', borderRadius: 16, padding: 28, maxWidth: 360, width: '100%' }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontWeight: 300, color: '#F8F7F5', margin: '0 0 16px' }}>Reject Image</p>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => { setRejectTarget(null); setRejectReason(''); }} style={{ height: 44, padding: '0 20px', background: 'transparent', border: '1px solid rgba(248,247,245,0.1)', borderRadius: 100, fontFamily: "'Jost',sans-serif", fontSize: 9, color: 'rgba(248,247,245,0.4)', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#F8F7F5', margin: 0 }}>Image Approvals <span style={{ fontSize: 18, color: images.length > 0 ? '#C9A84C' : '#888580' }}>({images.length} pending)</span></p>
          <button onClick={load} style={{ height: 36, padding: '0 14px', background: 'transparent', border: '1px solid rgba(248,247,245,0.1)', borderRadius: 8, fontFamily: "'Jost',sans-serif", fontSize: 9, fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(248,247,245,0.4)', cursor: 'pointer' }}>Refresh</button>
        </div>
      </div>

      {loading ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
          {[1,2,3,4,5,6].map(i => <div key={i} style={{ height: 200, borderRadius: 12, background: 'linear-gradient(90deg,#1E1C1A 25%,#2A2825 50%,#1E1C1A 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />)}
        </div>
      ) : images.length === 0 ? (
        <div style={{ background: '#161412', border: '1px solid rgba(248,247,245,0.1)', borderRadius: 12, padding: 60, textAlign: 'center' }}>
        ...
        Object.entries(byVendor).map(([vendorId, imgs]) => (
          <div key={vendorId} style={{ marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div>
                <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 300, color: '#F8F7F5', margin: '0 0 2px' }}>{imgs[0].vendor_name}</p>
                <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 8, fontWeight: 200, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(248,247,245,0.4)', margin: 0 }}>{imgs[0].vendor_category} · {imgs.length} image{imgs.length !== 1 ? 's' : ''}</p>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
              {imgs.map(img => (
                <div key={img.id} style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(248,247,245,0.1)', background: '#0C0A09' }}>
