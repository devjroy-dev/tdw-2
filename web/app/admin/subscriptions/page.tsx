    ...
    <>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#111', margin: 0 }}>Subscriptions</p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 500 }}>
          ...
          <tbody>
            {loading && [1,2,3,4].map(i => (
              <tr key={i}><td colSpan={5} style={{ padding: '12px 14px' }}><div style={{ height: 14, background: 'linear-gradient(90deg,#EEECE8 25%,#F4F2EE 50%,#EEECE8 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 4 }} /></td></tr>
            ))}
            ...
            {!loading && filtered.map(s => (
              <tr key={s.id} style={{ borderTop: '0.5px solid #F0EEE8' }}>
                <td style={{ padding: '11px 14px', fontFamily: "'DM Sans',sans-serif", fontWeight: 400, fontSize: 13, color: '#111' }}>{s.vendor_name || '—'}</td>
