  ...
  return (
    <>
      ...
      {showBackfillConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#FFFFFF', borderRadius: 16, padding: 28, maxWidth: 380, width: '100%' }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontWeight: 300, color: '#111', margin: '0 0 12px' }}>Backfill Entity Links?</p>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#111', margin: 0 }}>Data Tools</p>
      </div>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 14, padding: 24, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 300, color: '#111', margin: '0 0 6px' }}>Entity Links Backfill</p>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 14, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 300, color: '#111', margin: '0 0 4px' }}>Graph Stats</p>
        ...
        {statsLoading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[1,2,3,4,5].map(i => <div key={i} style={{ height: 40, borderRadius: 8, background: 'linear-gradient(90deg,#EEECE8 25%,#F4F2EE 50%,#EEECE8 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />)}
          </div>
        ) : linkStats.length === 0 ? (
            ...
            {linkStats.map(stat => {
              ...
              return (
                <div key={stat.link_type} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 400, color: '#111', margin: 0, width: 40, textAlign: 'right', flexShrink: 0 }}>{stat.count}</p>
                </div>
              );
            })}
          </div>
