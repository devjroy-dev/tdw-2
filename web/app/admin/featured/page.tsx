    ...
    <>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 32, color: '#111', margin: 0 }}>Featured Placement</p>
      ...
      {showSearch && (
        <div style={{ background: '#FFFFFF', border: '1px solid #C9A84C', borderRadius: 12, padding: 16, marginBottom: 20, position: 'relative' }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Type a maker name…" autoFocus style={{ width: '100%', border: 'none', borderBottom: '1px solid #E2DED8', background: 'transparent', fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 300, color: '#111', padding: '6px 0', outline: 'none' }} />
            ...
            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {filteredSearch.slice(0, 5).map(v => (
                <div key={v.id} onClick={() => toggleFeatured(v, false)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: '#F8F7F5', borderRadius: 8, cursor: 'pointer' }}>
                  <div>
                    <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 400, color: '#111', margin: '0 0 2px' }}>{v.name}</p>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[1,2,3].map(i => <div key={i} style={{ height: 64, borderRadius: 12, background: 'linear-gradient(90deg,#EEECE8 25%,#F4F2EE 50%,#EEECE8 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />)}
        </div>
      ) : featured.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {featured.map((v, i) => (
            <div key={v.id} style={{ background: '#FFFFFF', border: '1px solid #E2DED8', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 400, color: '#111', margin: '0 0 2px' }}>{v.name}</p>
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 300, color: '#888580', margin: 0 }}>{v.category}{v.city ? ` · ${v.city}` : ''}</p>
              </div>
