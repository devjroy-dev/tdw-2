...
export default function DreamersPage() {
  ...
  const metaRow = (label: string, value: string) => (
    <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '0.5px solid #F0EEE8' }}>
      ...
      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, fontWeight: 300, color: '#111', margin: 0 }}>{value}</p>
    </div>
  );
  ...
  return (
    <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 340, background: '#FFF', borderLeft: '1px solid #E2DED8', zIndex: 301, overflowY: 'auto' }}>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <button onClick={() => updateDreamer(selected.id, { discover_enabled: !selected.discover_enabled })} style={{ height: 40, background: '#F4F1EC', border: 'none', borderRadius: 8, fontFamily: 'Jost, sans-serif', fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#111', cursor: 'pointer' }}>
                  {selected.discover_enabled ? 'Disable Discovery' : 'Enable Discovery'}
                </button>
                <button onClick={() => { const n = prompt('Tokens to add:'); if (n && !isNaN(Number(n))) updateDreamer(selected.id, { token_balance: (selected.token_balance || 0) + Number(n) }); }} style={{ height: 40, background: '#F4F1EC', border: 'none', borderRadius: 8, fontFamily: 'Jost, sans-serif', fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#111', cursor: 'pointer' }}>
                  + Add Tokens
                </button>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or phone…" style={{ flex: 1, minWidth: 200, border: 'none', borderBottom: '1px solid #E2DED8', background: 'transparent', fontFamily: 'DM Sans, sans-serif', fontSize: 13, fontWeight: 300, color: '#111', padding: '8px 0', outline: 'none' }} />
      ...
      {/* Table */}
      <div style={{ background: '#FFF', border: '1px solid #E2DED8', borderRadius: 12, overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
          ...
          <tbody>
            ...
            {!loading && dreamers.map(d => (
              <tr key={d.id} onClick={() => setSelected(d)} style={{ borderTop: '0.5px solid #F0EEE8', cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget as HTMLTableRowElement).style.background = '#FAFAF8'} onMouseLeave={e => (e.currentTarget as HTMLTableRowElement).style.background = 'transparent'}>
                <td style={{ padding: '11px 14px', fontFamily: 'DM Sans, sans-serif', fontWeight: 400, fontSize: 13, color: '#111', whiteSpace: 'nowrap' }}>{d.name || '—'}</td>
